...
def main():
    ...
    for mode in Mode:
        ...
        print("time   {:>15} {:>21} {:>17}".format(round(no_of_runs[0]*run_times[0]),
              round(no_of_runs[1]*run_times[0]), round(no_of_runs[2]*run_times[1])))
        print(f"\nsec per iter    ")
...
class Optimizer():
    ...
    def _quantizier_main(self,rec_type, in_rec):
        if rec_type==RecordType.lQ_RESUME:
            ...
            quantizier = LayerQuantizier(in_rec, self.init_acc, self.ps, self.max_acc_loss, self.ones_range, resume_param_path)
        else:
    ...
    def eval_run_time(self, mode, no_of_tries=5):
        ...
        if mode==Mode.MAX_GRANULARITY:
            second_lvl_runs = sum([recs_first_lvl.no_of_channels[l]*recs_first_lvl.no_of_patterns[l] for l in range(recs_first_lvl.no_of_layers)])
    ...
    def _init_nn(self):
        self.nn.net.disable_spatial_layers(list(range(len(self.nn.net.generate_spatial_sizes(self.x_shape)))))
        self.nn.net.initialize_spatial_layers(self.x_shape, cfg.BATCH_SIZE, self.ps)
        _, test_acc, correct = self.nn.test(self.test_gen)
        self.nn.net.reset_ops()
        print(f'==> Asserted test-acc of: {test_acc} [{correct}]\n')
        #assert round(test_acc,2) == round(self.init_acc,2), f'starting accuracy does not match! curr_acc:{test_acc}, prev_acc{test_acc}'
