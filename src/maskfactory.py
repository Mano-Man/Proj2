...
def change_one_patch2d(mask, patch_n, patch_m, patch_size, p):
    mask[patch_n * patch_size:patch_n * patch_size + patch_size,
    patch_m * patch_size:patch_m * patch_size + patch_size] = p
    return mask


def change_one_patch3d(mask, patch_n, patch_m, patch_size, p, c):
    mask[c, patch_n * patch_size:patch_n * patch_size + patch_size,
    patch_m * patch_size:patch_m * patch_size + patch_size] = p
    return mask
            ...
            for ii in range(ii_start, patch_n):
                ...
                for jj in range(jj_start, patch_m):
                    ...
                    for p_idx in range(resume_params[3], patterns.shape[2]):
                        ...
                        yield layer, 0, ii * patch_n + jj, p_idx, uniform_layer(C, change_one_patch2d(mask, ii, jj,
                                                                                                      patch_size,
                                                                                                      patterns[:, :,
                                                                                                      p_idx]))
        elif mode == Mode.UNIFORM_PATCH:
                ...
                for ii in range(ii_start, new_patch_n):
                    ...
                    for jj in range(jj_start, new_patch_m):
                        ...
                        for p_idx, p in uniform_mask2d(new_patch_size, new_patch_size, patch_size, patterns,
                                                       p_start=resume_params[3]):
                            resume_params[3] = 0
                            mask = np.ones((C, new_patch_n * new_patch_size, new_patch_m * new_patch_size),
                                           dtype=mask_type)
                            mask = change_one_patch3d(mask, ii, jj, new_patch_size, p, channel)
                            yield layer, channel, ii * new_patch_n + jj, p_idx, mask[:, 0:(patch_n * patch_size),
                                                                                0:(patch_m * patch_size)]
